...
console.log('\n1 · главная в светлой и тёмной');
{
  ...
  for (const схема of ['light', 'dark']) {
    ...
    схемы[схема] = await page.evaluate(() => {
      ...
      return {
        ...
        // ⚠️ Контраст считается ЗДЕСЬ, в браузере, а не по значениям из файлов
        // темы: правило перебивают другие таблицы стилей, и по исходнику цвет
        // кнопки предсказать нельзя — 19.09 в исходнике стояло одно, а на экране
        // было другое. Фон берётся по дереву вверх: у самой подписи он прозрачный.
        контраст: (() => {
          const канал = (v) => {
            const c = v / 255;
            return c <= 0.03928 ? c / 12.92 : ((c + 0.055) / 1.055) ** 2.4;
          };
          const яркость = ([r, g, b]) => 0.2126 * канал(r) + 0.7152 * канал(g) + 0.0722 * канал(b);
          const разбор = (s) => (s.match(/\d+(\.\d+)?/g) || []).slice(0, 4).map(Number);
          const непрозрачный = (el) => {
            for (let n = el; n; n = n.parentElement) {
              const ц = разбор(getComputedStyle(n).backgroundColor);
              if (ц.length >= 3 && (ц[3] === undefined || ц[3] > 0.95)) return ц;
            }
            const ц = разбор(getComputedStyle(document.body).backgroundColor);
            return ц.length >= 3 ? ц : [255, 255, 255];
          };
          const итог = {};
          for (const с of ['.tt-bar__cta', '.tt-bar__link']) {
            const el = document.querySelector(с);
            if (!el) continue;
            // Свой фон элемента — отдельно: у залитой кнопки он обязан быть
            // непрозрачным. 19.09 токен не скомпилировался, заливка пропала, и
            // проверка контраста дала 15.7 — текст на фоне страницы. Сторож был
            // зелёным на сломанной кнопке.
            итог[с + ':свойФон'] = getComputedStyle(el).backgroundColor;
            const текст = разбор(getComputedStyle(el).color);
            const фон = непрозрачный(el);
            const [a, b] = [яркость(текст), яркость(фон)].sort((x, y) => y - x);
            итог[с] = Math.round(((a + 0.05) / (b + 0.05)) * 100) / 100;
          }
          return итог;
        })(),
  ...
  // Подпись обязана читаться на своём фоне в ОБЕИХ схемах. Порог AA — 4.5:
  // кегль кнопки 13.5 px, это обычный текст, послабления для крупного не будет.
  // 19.09 дневная схема давала 4.25, и владелец увидел сплошную плашку.
  const порог = 4.5;
  // ⚠️ Кнопки «Войти» у ВОШЕДШЕГО нет — там ссылка на аккаунт. Прогон заслона
  // идёт под сотрудником (предпросмотр темы иначе не открыть), поэтому её замер
  // там неприменим, а не провален. Контроль ниже не даёт пропустить ВСЕ элементы
  // разом: хотя бы один обязан быть измерен, иначе это сломанный пробник.
  let измерено = 0;
  for (const селектор of ['.tt-bar__cta', '.tt-bar__link']) {
    const св = s.контраст[селектор];
    const тм = d.контраст[селектор];
    if (!(св > 0) || !(тм > 0)) {
      пропуск(`${селектор}: контраст`, `элемента нет на странице (светлая ${св}, тёмная ${тм})`);
      continue;
    }
    измерено += 1;
    проверка(`${селектор}: подпись читается в обеих схемах (>= ${порог})`,
      св >= порог && тм >= порог, `светлая ${св}, тёмная ${тм}`);
    if (селектор === '.tt-bar__cta') {
      // ⚠️ Кнопка «Войти» повторяет шапку САЙТА, а там она «призрачная»: в покое
      // без заливки. Замер 19.09 обеих шапок: фон полос совпадает, а кнопка
      // расходилась — форум рисовал акцентную плашку. Проверяем именно это:
      // в покое фона нет, а подпись всё равно читается (контраст выше).
      // Прозрачность проверяем БЕЗ регулярного выражения: экранирование в ней при
      // правке файла уже съедалось, и детектор молча переставал совпадать.
      const прозрачный = (v) => !v || v.replace(/\s+/g, '') === 'rgba(0,0,0,0)' || v === 'transparent';
      проверка('кнопка входа без заливки, как в шапке сайта',
        прозрачный(s.контраст['.tt-bar__cta:свойФон']) && прозрачный(d.контраст['.tt-bar__cta:свойФон']),
        `светлая ${s.контраст['.tt-bar__cta:свойФон']}, тёмная ${d.контраст['.tt-bar__cta:свойФон']}`);
    }
  }
  проверка('контраст полосы вообще измерен (контроль)', измерено > 0, `элементов ${измерено}`);
...
console.log('\n8 · закрытость от поиска');
{
  ...
  проверка('googlebot проходит на форум', r.status() === 200, `HTTP ${r.status()}`);
}
...
console.log('\n14 · «Поделиться» несёт язык читателя');
{
  /*
    ...
    ⚠️ Читается ЖИВАЯ ссылка: ядро ставит её в `href` даты поста
    (`post/meta-data/date.gjs`), а ссылку темы показывает окно «Поделиться».
    Разбор кода темы ничего не доказал бы — геттеры ядра могут переехать, и язык
    молча перестанет попадать в ссылку.

    ⚠️ Первая редакция сторожа искала `[data-share-url]` — такого в разметке нет
    вовсе, и «ссылок без языка» оказалось ноль из нуля. Поймал это КОНТРОЛЬ
    «ссылки на месте»; без него проверка была бы зелёной на пустоте.
  */
  ...
  for (const [путь, переведена] of [["/t/17", true], ["/t/10", false]]) {
    ...
    проверка(`${путь}: посты и их ссылки на месте (контроль)`,
      r.постов > 0 && r.ссылки.length > 0 && r.язык.startsWith("ru"),
      `постов ${r.постов}, ссылок ${r.ссылки.length}, язык ${r.язык}`);
    const языки = [...new Set(r.ссылки.map(языкСсылки))];

    // Ссылка ТЕМЫ живёт в окне «Поделиться» — её собирает другой геттер.
    let темаСсылка = null;
    const кнопка = page.locator('.topic-footer-main-buttons button.share-and-invite, .topic-footer-main-buttons button.share').first();
    if (await кнопка.count()) {
      await кнопка.click();
      await page.waitForTimeout(1500);
      темаСсылка = await page
        .locator('.share-topic-modal input, .share-panel input, .modal-body input[type="text"]')
        .first()
        .inputValue()
        .catch(() => null);
    }
    ...
    if (переведена) {
      ...
      if (темаСсылка) {
        проверка(`${путь}: ссылка темы несёт tl=ru`, языкСсылки(темаСсылка) === "ru", темаСсылка);
      } else {
        пропуск(`${путь}: ссылка темы несёт tl=ru`, "окно «Поделиться» не открылось — гостю оно может быть недоступно");
      }
    } else {
      проверка(`${путь}: ссылка поста без языка`, языки.length > 0 && языки.every((l) => l === null), языки.join(","));
      if (темаСсылка) {
        проверка(`${путь}: ссылка темы без языка`, языкСсылки(темаСсылка) === null, темаСсылка);
      }
    }
    await ctx.close();
  }
