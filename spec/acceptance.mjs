/*
  ...
  Чего НЕ ловит: поломку ДО выката. Для этого нужен отдельный экземпляр.

  Запуск:  node spec/acceptance.mjs
...
const открыть = async (browser, путь, опции = {}) => {
  ...
  if (опции.locale) {
    await ctx.addCookies([
      { name: 'locale', value: опции.locale.split('-')[0], domain: new URL(БАЗА).hostname, path: '/' },
    ]);
  }
  const page = await ctx.newPage();
  const ошибки = [];
  page.on('pageerror', (e) => ошибки.push(String(e).slice(0, 140)));
  await page.goto(БАЗА + путь, { waitUntil: 'domcontentloaded', timeout: 60000 });
